...
def solve_LP_by_pulp(n, m, dests, flow_num, packet_num, router_path, egress_port, source_timing):
        ...
        for d in dests[s]:
            for f in range(flow_num[(s, d)]):
                for p in range(packet_num[(s, d, f)] - 1):
                    constraint_num += 1
                    LP += sender_timing[(s, d, f, p)] + 1 <= sender_timing[(s, d, f, p + 1)], "Constraint %d" % constraint_num
        ...
        for i in range(len(combs)):
            for j in range(i + 1, len(combs)):
                ...
                sdiff[(s, d1, f1, p1, d2, f2, p2)] = LpVariable("sdiff[(%d, %d, %d, %d, %d, %d, %d)]" % (s, d1, f1, p1, d2, f2, p2),
                                                                0, 1, 'Integer')
                constraint_num += 1
                LP += sender_timing[(s, d1, f1, p1)] - sender_timing[(s, d2, f2, p2)] <= -1 + 10E4 * sdiff[(s, d1, f1, p1, d2, f2, p2)], "Constraint %d" % constraint_num
                constraint_num += 1
                LP += sender_timing[(s, d1, f1, p1)] - sender_timing[(s, d2, f2, p2)] >= 1 - 10E4 * (1 - sdiff[(s, d1, f1, p1, d2, f2, p2)]), "Constraint %d" % constraint_num
                ...
                if (s1, d1, f1, r) in egress_port and (s2, d2, f2, r) in egress_port and egress_port[(s1, d1, f1, r)] == egress_port[(s2, d2, f2, r)]:
                    ...
                    for p1 in range(packet_num[(s1, d1, f1)]):
                        for p2 in range(packet_num[(s2, d2, f2)]):
                            ...
                            LP += router_timing[(s1, d1, f1, p1, r, e)] - router_timing[(s2, d2, f2, p2, r, e)] <= -1 + 10E4 * ediff[(s1, d1, f1, p1, s2, d2, f2, p2, r, e)]
                            LP += router_timing[(s1, d1, f1, p1, r, e)] - router_timing[(s2, d2, f2, p2, r, e)] >= 1 - 10E4 * (1 - ediff[(s1, d1, f1, p1, s2, d2, f2, p2, r, e)])
